...
export function RunsViewsMenu({
  ...
  const standardView: RunsSavedView = {
    id: "standard",
    name: "Standard",
    query: STANDARD_VIEW_QUERY,
    layout: {
  ...
  function updateViews(next: RunsSavedView[]) {
    onViewsChange(next);
  }
  ...
  function applyView(view: RunsSavedView) {
    query.applyViewQuery(view.query);
  ...
  function beginRename(view: RunsSavedView) {
    setName(view.name);
        ...
        {editingId ? (
          ...
          <>
            <DropdownMenuItem
              ...
              className={cn(
                areRunsViewsEqual(standardView, current) &&
            ...
            >
              <Check
                className={cn(
                  "size-4",
                  !areRunsViewsEqual(standardView, current) && "invisible",
                )}
              />
              <span className="min-w-0 flex-1 truncate">Standard</span>
            </DropdownMenuItem>
            {views.map((view) => {
              const active = areRunsViewsEqual(view, current);
              return (
                <div
                  key={view.id}
                  role="menuitem"
