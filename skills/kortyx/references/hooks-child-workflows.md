...
Studio currently receives generic `kortyx.workflow.call` spans. A dedicated invocation tree and observed call graph require Studio support; do not promise those views or invent static topology relationships. Publish the workflow catalog and check runtime telemetry separately. Public implementation guide: `https://kortyx.io/docs/guides/child-workflows` (when the containing SDK/docs release is available).
