...
- Within one node, await calls sequentially or use `parallel([useWorkflow(...), useWorkflow(...)])` from `kortyx`. It preserves tuple inference and input order. Await each group before the next group in the same node. Children can use their own nested groups. Native `Promise.all` child calls remain unsupported. Acyclic parallel calling/called graphs support ordinary child calls with isolated node activation journals; parallel graph back-edges and branch `transitionTo` handoffs are rejected.
...
- Use ordinary `useInterrupt` or `useReason({ interrupts: { contracts } })` in children. Contract name, custom request, schema identity, and structured response bridge through the parent's existing transport. The singular `interrupt` form is deprecated and removed next major.
