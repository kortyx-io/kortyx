...
## Automatic model diagnostics follow-up

Model provider failures and `useReason` decision/output parsing failures now automatically retain bounded type/message. The shared diagnostic projector does not change client-facing failure descriptors. Studio selects failed model spans instead of generation-completion metadata, and surfaces output-processing failures on a Model reasoning row when no child provider failure already explains the failure. Paused/cancelled reasoning spans stay hidden as control flow. Both trace adapters support optional model diagnostic replacement/suppression; callbacks never change execution. OpenTelemetry ignores repeated failure observations after a physical span ends.

Verified 28 package test tasks, 28 type-check tasks and 8 scoped coverage tasks. All four disposable-install browser journeys pass. The new journey executes a caught provider TypeError and a caught `INVALID_MODEL_JSON` failure after a normally stopped provider response, with both root workflows Completed. Studio shows the provider type/message and the parser type/message/code. The entire authenticated Studio run-detail API response excludes private prompt/output, exception cause/custom field/credential, provider body/metadata and raw usage markers. Failed-generation events keep normalized usage counts but omit arbitrary provider metadata and raw usage. No live provider traffic or publication was performed; auto-merge and the release heartbeat remain disabled.
