...
`maintenance.setup()` runs the bundled, ordered SQL migrations from `packages/runtime/src/framework/postgres/migrations/`. Each numbered TypeScript file contains plain SQL, bundled into the SDK without filesystem reads or an extra migration library. Released migration definitions are immutable; future changes add a new consecutive version and register it in `runtimeMigrations`. The ledger stores a SHA-256 checksum of each migration SQL string. Editing applied SQL makes setup reject before any pending migration; restore the original definition and add a new version instead. Checksums detect migration-definition drift, not manual changes to tables or historical workflow JSON.

Each migration runs in its own transaction: acquire the schema advisory lock, bootstrap the version ledger if needed, validate that recorded versions form a supported consecutive prefix, execute the next pending SQL, and record its version before committing. History is checked again under the lock for every version, so concurrent setup calls cannot apply the same migration twice. Newer schemas and inconsistent/gapped histories are rejected before pending SQL executes. Original version-only v1 ledgers adopt the current v1 checksum once without rerunning initial schema SQL. Their historical SQL cannot be verified retroactively; missing checksums in an already-updated ledger are rejected.

A failed migration rolls back its SQL and version record; earlier successful migrations remain committed. A failure during initial setup also rolls back the new ledger and partial tables. Setup stops on failure and a later retry resumes at the first unapplied version. There are no automatic down migrations or application-request migrations. The deployment command must complete setup before traffic and propagate failures.

Advisory and DDL lock acquisition is limited to five seconds per lock, preserving a stricter database-configured `lock_timeout`. The setting is transaction-local and cannot leak into application queries. A timeout rolls back the current migration; deployments should retry after contention clears. This is not a statement execution timeout: large backfills and index builds still require workload planning and a deployment-configured `statement_timeout`.

Migration SQL must be compatible with PostgreSQL transactions; it must not contain transaction-control commands or operations such as `CREATE INDEX CONCURRENTLY`. For rolling deployments, use additive changes and compatible backfills first, and defer destructive changes until older instances no longer depend on the old shape. Workflow/state JSON compatibility remains a separate application contract. The runner does not perform backend data transfer or modify application-owned tables.

### Deployment and OSS comparison

- [LangGraph PostgreSQL saver](https://github.com/langchain-ai/langgraph/blob/main/libs/checkpoint-postgres/langgraph/checkpoint/postgres/base.py) bundles an ordered SQL list and tracks migration versions. This supports keeping a small SDK-owned runner rather than adding an ORM. Its list also includes concurrent index creation; our current transactional runner deliberately excludes that operation. A future large-table index migration needs a separately designed, resumable deployment path, not a silent switch to nontransactional SQL.
- [node-pg-migrate](https://salsita.github.io/node-pg-migrate/cli) validates ordering, records history, and uses advisory locks. Its default combines pending migrations into one transaction; we commit each version separately so a failed later version can resume without repeating earlier work. Both approaches require explicit deployment coordination.
- [Flyway validation](https://documentation.red-gate.com/flyway/reference/commands/validate) compares stored checksums with migration definitions. We adopt checksum validation without introducing a general-purpose migration tool or automatic history repair.
- [PostgreSQL lock timeouts](https://www.postgresql.org/docs/17/runtime-config-client.html#GUC-LOCK-TIMEOUT) bound acquisition waits, including implicit DDL locks. They do not make migration execution or maintenance free of database contention.

Use the same database schema/search path in deployment and runtime roles, and keep it stable across upgrades. Setup requires permission to create/alter the SDK tables; ordinary runtime credentials can be restricted to the required data operations after setup. Namespace isolates runtime records, not authorization: tenant access control remains in the application. All instances sharing a namespace should use the same retention configuration, because a shorter policy can physically delete history needed by an instance with a longer policy. PostgreSQL backups and autovacuum remain infrastructure responsibilities; retention deletion is not secure erasure of backups. External side effects still require application idempotency, and replay of old snapshots requires compatible workflow code.
