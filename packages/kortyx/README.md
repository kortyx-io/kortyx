...
Configured Studio/OpenTelemetry tracing also captures model provider failures and `useReason` output-parsing failures automatically, including bounded error type/message. No extra wiring is needed. Studio opens the failed model event and displays reasoning/output failures separately when the model response itself completed normally. Prompts, outputs, exception stacks/causes and raw provider responses are excluded by default. Messages are verbatim; the optional trace adapter `error(error)` override can return `{type, message}` or `null` to replace/suppress model diagnostics without changing execution. Client-facing failure contracts remain unchanged.
