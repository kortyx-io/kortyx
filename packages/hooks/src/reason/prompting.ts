...
export const withStructuredStreamHints = (
  ...
  for (const path of args.setFieldPaths ?? []) {
    rules.push(
      `- For the field path pattern \`${path}\`, write one stable final value and do not rewrite it after it is complete.`,
    );
  }

  for (const path of args.appendFieldPaths ?? []) {
    rules.push(
      `- For the array field path pattern \`${path}\`, write items in final order and finish each item before starting the next.`,
    );
    rules.push(
  ...
  for (const path of args.textDeltaFieldPaths ?? []) {
    rules.push(
      `- For the string field path pattern \`${path}\`, write the value progressively by appending text to the end.`,
    );
    rules.push(
