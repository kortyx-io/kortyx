...
const includesText = (value: string | null | undefined, filter?: string) =>
  !filter ||
  Boolean(value?.toLowerCase().includes(filter.trim().toLowerCase()));

const withinRange = (isoValue: string, query: StudioListQuery): boolean => {
  const time = Date.parse(isoValue);
  if (!Number.isFinite(time)) return false;
  if (query.range === "Custom range") {
    const after = query.startedAfter
      ? Date.parse(query.startedAfter)
      : Number.NEGATIVE_INFINITY;
    const before = query.startedBefore
      ? Date.parse(query.startedBefore)
...
const paginate = <T>(items: T[], query: StudioListQuery): StudioListPage<T> => {
...
export const listStudioRuns = async (
  ...
  const models = await getStudioReadModels(db, input);
  ...
  const statusOrder: StudioRunStatus[] = [
    "running",
    "completed",
    "interrupted",
    "failed",
  ...
  const rows = models.runs
    .filter(
      (run) =>
        (!query.env ||
          query.env === "All environments" ||
          run.environment === query.env) &&
        (!statuses.length || statuses.includes(run.status)) &&
        (!providers.length ||
          (run.provider !== null && providers.includes(run.provider))) &&
        (!queryBoolean(query.tool) || run.hasTool) &&
        withinRange(run.startedAt, query) &&
        (!query.workflow ||
          run.workflowRefs.some((ref) =>
            includesText(ref.workflowId, query.workflow),
          )) &&
        (!query.version ||
          run.workflowRefs.some(
            (ref) =>
              (!query.workflow ||
                includesText(ref.workflowId, query.workflow)) &&
              includesText(ref.declaredVersion ?? "unversioned", query.version),
          )) &&
        (!query.transition ||
          run.transitionIds.some((id) => includesText(id, query.transition))) &&
        includesText(run.path.join(" "), query.path) &&
        includesText(run.sessionId, query.session) &&
        includesText(run.model, query.model) &&
        includesText(run.result, query.result) &&
        (!minimumCost || (run.cost ?? -1) >= minimumCost) &&
        (!minimumDurationMs || (run.durationMs ?? -1) >= minimumDurationMs) &&
        (!minimumTokens || (run.tokens ?? -1) >= minimumTokens) &&
        (!needle ||
          [
            run.id,
            run.sessionId,
            ...run.workflowIds,
            ...run.workflowRefs.flatMap((ref) => [
              ref.workflowId,
              ref.declaredVersion,
            ]),
            ...run.transitionIds,
    ...
    .sort((a, b) => {
      const sort = query.sort ?? "started";
      const values: Record<string, [number, number]> = {
        started: [Date.parse(a.startedAt), Date.parse(b.startedAt)],
        duration: [a.durationMs ?? -1, b.durationMs ?? -1],
        tokens: [a.tokens ?? -1, b.tokens ?? -1],
        cost: [a.cost ?? -1, b.cost ?? -1],
        status: [statusOrder.indexOf(a.status), statusOrder.indexOf(b.status)],
      };
      const [first, second] = values[sort] ?? [
        Date.parse(a.startedAt),
        Date.parse(b.startedAt),
      ];
      return query.dir === "asc" ? first - second : second - first;
    });
...
export const listStudioSessions = async (
  ...
  const minimumDuration = queryNumber(query.minDuration) * 1_000;
  const maximumDuration = queryNumber(query.maxDuration) * 1_000;
  const statusOrder: StudioRunStatus[] = [
    "running",
    "completed",
    "interrupted",
  ...
  const rows = models.sessions
    .filter(
      (session) =>
        (!query.env ||
          query.env === "All environments" ||
          session.environment === query.env) &&
        (!statuses.length || statuses.includes(session.status)) &&
        (!queryBoolean(query.error) || Boolean(session.latestError)) &&
        (!queryBoolean(query.interrupt) ||
          Boolean(session.pendingInterruptId || session.interrupted)) &&
        (!queryBoolean(query.checkpoint) || session.checkpoints > 0) &&
        (!queryBoolean(query.fork) || session.hasFork) &&
        (!query.workflow ||
          session.workflowIds.some((id) => includesText(id, query.workflow))) &&
        includesText(session.userId, query.user) &&
        includesText(session.tenantId, query.tenant) &&
        includesText(session.providers.join(" "), query.provider) &&
        includesText(session.models.join(" "), query.model) &&
        includesText(session.tags.join(" "), query.tags) &&
        (!minimumCost || (session.cost ?? -1) >= minimumCost) &&
        (!maximumCost ||
          (session.cost ?? Number.POSITIVE_INFINITY) <= maximumCost) &&
        (!minimumTokens || (session.tokens ?? -1) >= minimumTokens) &&
        (!maximumTokens ||
          (session.tokens ?? Number.POSITIVE_INFINITY) <= maximumTokens) &&
        (!minimumDuration || (session.durationMs ?? -1) >= minimumDuration) &&
        (!maximumDuration ||
          (session.durationMs ?? Number.POSITIVE_INFINITY) <=
            maximumDuration) &&
        withinRange(session.lastActivityAt, query) &&
        (!needle ||
          [
            session.id,
            session.userId,
    ...
    .sort((a, b) => {
      ...
      const values: Record<string, [number, number]> = {
        activity: [Date.parse(a.lastActivityAt), Date.parse(b.lastActivityAt)],
        duration: [a.durationMs ?? -1, b.durationMs ?? -1],
        tokens: [a.tokens ?? -1, b.tokens ?? -1],
        cost: [a.cost ?? -1, b.cost ?? -1],
        runs: [a.runs, b.runs],
...
export const listStudioInterrupts = async (
  ...
  const needle = query.q?.trim().toLowerCase();
  const minimumAge = queryNumber(query.minAge) * 60_000;
  const maximumAge = queryNumber(query.maxAge) * 60_000;
  const statusOrder: StudioInterruptStatus[] = [
    "pending",
    "resolved",
    "expired",
    "failed",
  ...
  const rows = models.interrupts
    .filter((interrupt) => {
      const age = Date.now() - Date.parse(interrupt.createdAt);
      return (
        (!query.env ||
          query.env === "All environments" ||
          interrupt.environment === query.env) &&
        (!statuses.length || statuses.includes(interrupt.status)) &&
        (!types.length || types.includes(interrupt.type)) &&
        (!outcomes.length ||
          (interrupt.resumeOutcome !== null &&
            outcomes.includes(interrupt.resumeOutcome))) &&
        (!queryBoolean(query.error) || Boolean(interrupt.resumeError)) &&
        includesText(interrupt.workflowId, query.workflow) &&
        includesText(interrupt.nodeId, query.node) &&
        includesText(interrupt.sessionId, query.session) &&
        includesText(interrupt.userId, query.user) &&
        includesText(interrupt.tenantId, query.tenant) &&
        includesText(interrupt.resolvedBy, query.resolver) &&
        (!minimumAge || age >= minimumAge) &&
        (!maximumAge || age <= maximumAge) &&
        withinRange(interrupt.createdAt, query) &&
        (!needle ||
          [
            interrupt.id,
    ...
    .sort((a, b) => {
      ...
      if (sort === "priority") {
        value =
          ...
          (a.status === "pending" ? createdA - createdB : createdB - createdA);
      } else {
        const values: Record<string, [number, number]> = {
        ...
        const [first, second] = values[sort] ?? [createdA, createdB];
        value = first - second;
      }
      return query.dir === "desc" ? -value : value;
    });
  return paginate(rows, query);
};
